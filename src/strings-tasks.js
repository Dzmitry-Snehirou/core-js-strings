...
function removeFirstOccurrences(str, value) {
  ...
  const firstStr = str.substring(0, index);
  const lastIdex = index + value.length;
  const lastStr = str.substring(lastIdex);
...
function isPalindrome(str) {
  const clstr = str.toLowerCase().replace(/[^a-zA-Z0-9]+/g, '');
  const reverse = clstr.split('').reverse().join('');
  return reverse === clstr;
}
/**
